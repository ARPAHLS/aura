...
| [audit_pipeline.py](audit_pipeline.py) | Two sessions, audit report, compare, OTel export assert, hash-chain verify, CLI follow-ups | Show the full export slice with MockSkill host | You need receipt + compare + verify without Skillware live | Missing write permissions under sessions dir | Agent ref, mock skill manifest, `AURA_HOME` |
