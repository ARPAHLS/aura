...
- **Examples layout** — all runnable demos live in numbered folders `examples/01-*` … `examples/13-*` with `main.py` + `README.md` (moved former flat scripts into `01-minimal-loop`, `02-guarded-tools`, `03-task-mode`, `04-sequencer-pipeline`, `09-audit-pipeline`; operator identity → `13-operator-identity`).
