...
- **Spine payload redaction** — `session.emit` always redacts secret-like keys (`token`, `api_key`, `password`, `authorization`, `*_secret`, …) and injected secret values on JSONL / summary / OTel. Usage counters (`tokens`, `token_count`) are kept. Applies even when `capabilities[]` is empty.
- **Examples layout** — all runnable demos live in numbered folders `examples/01-*` … `examples/14-*` with `main.py` + `README.md` (moved former flat scripts into `01-minimal-loop`, `02-guarded-tools`, `03-task-mode`, `04-sequencer-pipeline`, `09-audit-pipeline`; operator identity → `13-operator-identity`; capability broker → `14-capability-broker`).
- **Documentation** — removed internal backlog [docs/guides/skillware-follow-ups.md](docs/guides/skillware-follow-ups.md) from public docs; deferred work stays in [ROADMAP.md](docs/ROADMAP.md) and local `AURA_PLAN.md` / `issues/`; preset YAML consolidated under `observers.md` (trimmed duplicate sections in [skillware-integration.md](docs/skillware-integration.md)).
