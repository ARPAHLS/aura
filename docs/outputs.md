...
**Spine events (escalation):** When `profile.escalations[]` is set, trigger events (`slo.missed`, `conformance.drift`, `observer.alert`, `constraint.violated`) may emit `escalation.fired` and action events (`membrane.nudge`, `escalation.email`, `escalation.wake`). See [observers.md](observers.md#3-escalation-playbooks--profileescalations-47).
