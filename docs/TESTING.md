...
- **Host stress simulation:** `python scripts/aura_host_stress_sim.py` — thirteen scenarios (loose/tight/tailored coats, spectrum low/high bind, single/multi/chain Skillware paths, sequencer, observers, export compare). CI: `tests/test_host_stress_sim.py` (`@pytest.mark.skillware`).
